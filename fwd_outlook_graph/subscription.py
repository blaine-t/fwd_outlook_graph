...
def get_subscriptions():
    ...
    if response.status_code == 200:
        ...
        print(
            f"[{response.status_code}] Get subscriptions result: {response.text}")
...
def subscribe():
    ...
    if response.status_code == 201:
        ...
        print(f"[{response.status_code}] Subscription result: {response.text}")
...
def resubscribe(subscription_id):
    ...
    if response.status_code == 200:
        ...
        print(f"[{response.status_code}] Resubscription result: {response.text}")
...
def unsubscribe(subscription_id):
    ...
    if response.status_code == 204:
        ...
        print(f"[{response.status_code}] Unsubscription result: {response.text}")
